...
function runCli(command: string, cwd: string): Promise<{ stdout: string; stderr: string; code: number | null }> {
  return new Promise((resolve) => {
    output.appendLine(`> ${command}`);
    const child = exec(command, { cwd, maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
      if (err) {
...
function extractMembersFromText(text: string): string[] {
...
function findTypesFromPackageXml(workspaceRoot: string): { [typeName: string]: string[] } {
  const map: { [typeName: string]: string[] } = {};
...
async function gatherMetadataStrings(editor: vscode.TextEditor): Promise<string[] | undefined> {
  ...
  for (const sel of selections) {
    ...
    if (sel.isEmpty) {
      ...
      if (wordRange) text = doc.getText(wordRange);
      else text = doc.lineAt(sel.start.line).text.trim();
    } else {
    ...
    for (const sel of selections) {
      ...
      while ((mm = memberRegexGlobal.exec(docText)) !== null) {
        ...
        if (mStart >= selStart && mEnd <= selEnd) {
          ...
          if (insideIncluded) continue; // skip, it's already part of a full block
