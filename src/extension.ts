...
function writeTempManifest(manifestContent: string, prefix: string): string {
  ...
  const loc = config.get<string>('manifestExtension.tempLocation') || 'workspace';
  if (loc === 'extensionStorage') {
    if (!extensionContext) throw new Error('No extension context available for extensionStorage');
    const storageUri = extensionContext.globalStorageUri.fsPath;
    if (!fs.existsSync(storageUri)) fs.mkdirSync(storageUri, { recursive: true });
...
function runCliRaw(command: string, cwd: string): Promise<{ rawStdout: string; rawStderr: string; code: number | null }> {
  return new Promise((resolve) => {
    const child = exec(command, { cwd, maxBuffer: 50 * 1024 * 1024 }, (err, stdout, stderr) => {
...
async function runSfWithManifest(manifestPath: string, sfCmdBase: string): Promise<SfRunResult> {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        if (lastOpen !== -1 && lastClose !== -1 && lastClose > lastOpen) {
          ...
          try { parsed = JSON.parse(jsonText); } catch (ee) { parsed = undefined; }
        }
      }
      resolve({ parsed, cleaned, rawStdout: stdout, rawStderr: stderr, code: code });
    });
    child.on('error', (err) => {
      resolve({ parsed: undefined, cleaned: String(err), rawStdout: '', rawStderr: String(err), code: (err as any).code ?? 1 });
    });
  });
}

export function buildPackageFromText(docText: string, selectionRanges: Array<{ start: number; end: number }>): string | undefined {
  const blocks: string[] = [];
...
function presentSfResult(parsed: any | undefined, cleaned: string, successMsg: string, failMsg: string) {
  if (parsed) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const retrieve = vscode.commands.registerCommand('sfdxManifest.retrieve', async () => {
    ...
    await writeManifestAndRun(editor,
      'package-retrieve',
  ...
  async function writeManifestAndRun(editor: vscode.TextEditor, prefix: string, sfCmdBase: string, progressTitle: string, successMsg: string, failMsg: string) {
